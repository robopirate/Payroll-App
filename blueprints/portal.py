...
@portal_required
def portal_quick_attendance():
    """Mobile-optimized attendance marking for teachers/school heads."""
    emp = Employee.query.get(current_user.employee_id)
    today = date.today()

    # Get assigned schools
    schools = [s for s in emp.schools if s.is_active]
    if not schools:
        flash('You are not assigned to any school. Contact admin.', 'warning')
        return redirect(url_for('.portal_dashboard'))
    ...
    if not selected_school_id and len(schools) == 1:
        selected_school_id = schools[0].id
    ...
    if selected_school_id:
        ...
        # Only show employees assigned to this school who are active
        employees = [e for e in selected_school.assigned_employees if e.is_active]
        employees.sort(key=lambda e: e.name)
        existing = {a.employee_id: a for a in Attendance.query.filter_by(date=sel_date).all() if a.employee_id in [e.id for e in employees]}

    if request.method == 'POST':
        sel_date_str = request.form.get('date', sel_date.isoformat())
        try:
            sel_date = datetime.strptime(sel_date_str, '%Y-%m-%d').date()
        except ValueError:
            sel_date = today
        school_id = int(request.form.get('school_id', selected_school_id or 0))
        if school_id:
            from models import School
            school = School.query.get_or_404(school_id)
            school_emps = [e for e in school.assigned_employees if e.is_active]
            for e in school_emps:
                status = request.form.get(f'status_{e.id}', 'absent')
                att = Attendance.query.filter_by(employee_id=e.id, date=sel_date).first()
                if att:
                    att.status = status
                    att.admin_override = False
                else:
                    att = Attendance(employee_id=e.id, date=sel_date, status=status, admin_override=False)
                    db.session.add(att)
            db.session.commit()
            flash(f'Attendance saved for {sel_date.strftime("%d %b %Y")}.', 'success')
            return redirect(url_for('.portal_quick_attendance', date=sel_date.isoformat(), school_id=school_id))

    return render_template('portal/quick_attendance.html',
                           emp=emp, schools=schools, selected_school=selected_school,
                           employees=employees, existing=existing,
                           sel_date=sel_date, today=today)
